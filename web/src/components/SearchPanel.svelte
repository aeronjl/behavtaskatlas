<script lang="ts">
  ...
  function highlight(text: string, tokens: string[]): string {
    ...
    return text.replace(re, "<mark class=\"bg-warn-soft\">$1</mark>");
  }
...
<div class="space-y-3">
  ...
  <div class="flex flex-wrap items-center gap-2 text-xs text-fg-muted">
    ...
    {#each ALL_TYPES as t (t)}
      ...
      <button
        ...
        class:text-fg-secondary={!active}
        class:border-rule-strong={!active}
    ...
    {#if activeFilterCount > 0}
      <button
        type="button"
        class="rounded border border-rule-strong bg-surface px-2 py-0.5 font-semibold text-fg hover:border-accent hover:text-accent"
        onclick={clearAll}
      >
  ...
  {#if results.length === 0 && tokens.length > 0}
    <section class="rounded-md border border-rule bg-surface-raised p-4 text-sm text-fg-muted">
  ...
  {:else}
    <ul class="divide-y divide-rule rounded-md border border-rule bg-surface-raised">
      {#each results as { entry } (entry.id)}
        <li class="px-3 py-2 text-sm">
          <div class="flex items-baseline justify-between gap-3">
            ...
            <span class="rounded bg-surface-sunken px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-fg-muted">
          ...
          {/if}
          {#if entry.body}
            <p class="mt-0.5 line-clamp-2 text-xs text-fg-muted">
